...
--luacheck: defclass={tr:{_type:table,1:number,2:number,3:number},em:{_type:table,1:number,2:number,3:number},rad:number,flags:'bool[]'}
MaterialDef = defclass(MaterialDef)

function MaterialDef:init(transparency,emitance,radius,flags)
    local flags = flags --as:string[]
    local flg = {} --as:bool[]
    if flags then
...
materials = materials or {} --as:MaterialDef[][]
buildings = buildings or {} --as:MaterialDef[][][]
special = special or {}
items = items or {}
...
function addMaterial(id,transparency,emitance,radius,flags)
    ...
    materials[matinfo.type][matinfo.index]=MaterialDef(transparency,emitance,radius,flags)
end
function buildingLookUp(id)
    local tokens={} --as:string[]
    local lookup = { --as:number[][]
...
function addCreature(id,transparency,emitance,radius,flags)
    ...
    local mat=MaterialDef(transparency,emitance,radius,flags)
...
function makeMaterialDef(transparency,emitance,radius,flags)
    return MaterialDef(transparency,emitance,radius,flags)
end
...
------------------------------------------------------------------------
is_computer_quantum = is_computer_quantum or false -- will enable more costly parts in the future
--special things
special.LAVA=MaterialDef({0.8,0.2,0.2},{0.8,0.2,0.2},5)
special.WATER=MaterialDef({0.5,0.5,0.8})
special.FROZEN_LIQUID=MaterialDef({0.2,0.7,0.9}) -- ice
special.AMBIENT=MaterialDef({0.85,0.85,0.85}) --ambient fog
special.CURSOR=MaterialDef({1,1,1},{0.96,0.84,0.03},11, {"flicker"})
special.CITIZEN=MaterialDef(nil,{0.80,0.80,0.90},6)
...
--items
addItem("GEM",nil,nil,nil,{"useMaterial","onGround"})
addItem("ROUGH",nil,nil,nil,{"useMaterial","onGround"})
addItem("SMALLGEM",nil,nil,nil,{"useMaterial","onGround"})
return _ENV
