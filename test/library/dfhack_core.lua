...
function test.safe_index()
    -- Test basic indexing
    local t = {a = 1, b = {c = 2}}
    expect.eq(safe_index(t, 'a'), 1)
    expect.eq(safe_index(t, 'b', 'c'), 2)

    -- Test nil handling
    expect.eq(safe_index(nil, 'a'), nil)
    expect.eq(safe_index(t, 'nonexistent'), nil)
    expect.eq(safe_index(t, 'b', 'nonexistent'), nil)

    -- Test nested nil handling
    expect.eq(safe_index(t, 'b', 'c', 'd'), nil)
end
...
local function test_ensure_key()
    ...
    local result = ensure_key(t, 'new_key')
    expect.table_eq(result, {})
    expect.table_eq(t.new_key, {})

    -- Test existing key
    t.existing = {value = 1}
    result = ensure_key(t, 'existing')
    expect.eq(result, t.existing)
    expect.eq(t.existing.value, 1)

    -- Test with default value
    result = ensure_key(t, 'with_default', {default = true})
...
local function test_pos_functions()
    ...
    expect.eq(pos2xyz(invalid_pos), nil)

    -- Test xyz2pos
    local new_pos = xyz2pos(5, 10, 15)
    expect.eq(new_pos.x, 5)
    expect.eq(new_pos.y, 10)
    expect.eq(new_pos.z, 15)

    -- Test xyz2pos with nil
    local nil_pos = xyz2pos(nil, nil, nil)
    expect.eq(nil_pos.x, -30000)
    expect.eq(nil_pos.y, -30000)
    expect.eq(nil_pos.z, -30000)

    -- Test same_xyz
    expect.true_(same_xyz(pos, pos))
    expect.false_(same_xyz(pos, {x = 10, y = 20, z = 25}))

    -- Test pos2xy
    local x2, y2 = pos2xy(pos)
    expect.eq(x2, 10)
    expect.eq(y2, 20)

    -- Test xy2pos
    local pos2d = xy2pos(3, 7)
    expect.eq(pos2d.x, 3)
    expect.eq(pos2d.y, 7)

    -- Test same_xy
    expect.true_(same_xy(pos, pos))
    expect.true_(same_xy(pos, {x = 10, y = 20, z = 99}))
    expect.false_(same_xy(pos, {x = 10, y = 25, z = 30}))
end
