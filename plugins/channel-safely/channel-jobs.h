 ...
 * erase_and_cancel: finds a job corresponding to a coord, removes the mapping in jobs, and calls Job::removeJob, then returns an iterator following the element removed
...
public:
    ...
    void clear();
    Jobs::iterator erase_and_cancel(const df::coord &map_pos);
