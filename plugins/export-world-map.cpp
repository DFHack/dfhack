...
static auto create_outlines(std::vector<std::vector<df::coord2d>> components) {
...
static command_result export_regions(color_ostream &out)
{
    ...
    // once every region tile has been assigned the world tile it references for its metadata
    // everything else can be done in parallel
    std::mutex out_mutex;
    std::vector<std::ostringstream> out_lines;
    out_lines.reserve(world_tile_region.size());

    std::for_each(
        std::execution::par, world_tile_region.begin(), world_tile_region.end(),
        [&out_lines,&out_mutex](auto &wtr)
    {
        auto& [biome_tile, region] = wtr;
        ...
        auto paths = create_outlines(components);
        ...
        if (paths.size() == 1) {
            print_range(out_line, paths, print_path , "POLYGON(", ",", ")\n" );
        } else {
            print_range(out_line, paths, print_path , "MULTIPOLYGON((", "),(", "))\n" );
        }
