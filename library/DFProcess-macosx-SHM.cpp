...
{
  // I am not sure if we can get this info
  return 0;
}
...
bool SHMProcess::Private::validate(vector <memory_info *> & known_versions)
{
    ...
    // find the binary
    sprintf(lsof_cmd,"/usr/sbin/lsof -F -p %d", process_ID);
    lpipe = popen(lsof_cmd, "r");
    do {
        r = fgets(target_name, 1024, lpipe);
	if (r == NULL) {
	  perror("fgets");
	  pclose(lpipe);
	  return false;
    ...
    *r = 0;
    ...
    string hash = md5.getHashFromFile(target_name+1);
...
void SHMProcess::getMemRanges( vector<t_memrange> & ranges )
{
    ...
    sprintf(vmmap_cmd, "/usr/bin/vmmap -w -interleaved %lu", d->process_ID);
    FILE *mapFile = ::popen(vmmap_cmd, "r");
    
    while (fgets(buffer, 1024, mapFile))
    {
        // look for __DATA zones with 'dwarfort.exe' in the line
        // also look for MALLOC_* zones
...
bool SHMProcess::attach()
{
    ...
    * Locate the segment.
    */
    if ((d->shm_ID = shmget(SHM_KEY + d->process_ID, SHM_SIZE, 0600)) < 0)
    {
        perror("shmget");
        d->FreeLocks();
