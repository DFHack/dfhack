...
void visual_animation_managerst::clear_pending(viewport_animationst &state) {
    state.pending.clear();
    state.pending_frames = 0;
    state.pending_age = 0;
}

void visual_animation_managerst::abandon_pending(viewport_animationst &state) {
    state.movements.clear();
    clear_pending(state);
}

void visual_animation_managerst::reset_facing(viewport_animationst &state) {
    std::fill(state.facing.begin(), state.facing.end(), int8_t(native_sprite_facing));
    state.has_mirrored = false;
}

void visual_animation_managerst::reset_tracking(viewport_animationst &state) {
    abandon_pending(state);
    state.suppress_frames = 0;
}

uint64_t visual_animation_managerst::compute_buffer_signature(
...
double visual_animation_managerst::shift_match_ratio(const viewport_visual_animation_inputst &input,
...
std::array<int32_t, 2> visual_animation_managerst::shared_movement_delta(const int32_t *current,
...
visual_animation_managerst::viewport_animationst &
visual_animation_managerst::get_viewport(const viewport_visual_animation_inputst &input) {
...
float visual_animation_managerst::movement_progress(uint32_t start_time_ms) const {
    return animation_progress(frame_time_ms, start_time_ms, movement_duration_ms);
}

void visual_animation_managerst::begin_frame(uint32_t now_ms) {
...
void visual_animation_managerst::synchronize_viewport(
    const viewport_visual_animation_inputst &input) {
    ...
    // This hook runs per frame; the viewport is recomputed only when it changes,
    // and while paused hardly at all. Re-reading a landed scroll steps every
    // sprite by a tile.
    ...
    // On the crossing frame `current` is the new view and `previous` the old one,
    // so a sprite on each side, a tile apart, reads as one that moved between
    // them.
    const bool crossed_views = buffers_advanced && state.previous_view_stale;
    if (buffers_advanced)
        state.previous_view_stale = false;
    // The new view is drawn at the current window, so a queued scroll is already
    // in it. Left queued it would never match, and suppress everything until it
    // aged out.
    if (crossed_views)
        clear_pending(state);
    ...
    if (buffers_advanced && !crossed_views && !state.pending.empty()) {
        ...
        for (size_t count = 1; count <= state.pending.size(); ++count) {
            ...
            // yet". Accepting it would retire shifts the buffers have still to apply.
        ...
        if (!any_data) {
            ...
            state.movements.erase(std::remove_if(state.movements.begin(), state.movements.end(),
                                  ...
                                  state.movements.end());
            // Facing describes creatures still on screen, so translate it rather than
            // drop it.
    ...
    const bool suppress =
        !buffers_advanced || crossed_views || !state.pending.empty() || state.suppress_frames > 0;
    // The countdown measures redraws, not frames, so a repeated viewport must not
    // spend it.
    ...
    if (!suppress) {
        ...
        for (size_t layer = 0; layer < input.current.size(); ++layer) {
            ...
            for (int32_t x = 0; x < input.dim_x; ++x) {
                for (int32_t y = 0; y < input.dim_y; ++y) {
                    ...
                    if (texpos == 0)
                        continue;
                    if (static_cast<viewport_visual_layer>(layer) == viewport_visual_layer::item &&
                        ...
                        for (int32_t dx = -1; dx <= 1; ++dx) {
                            for (int32_t dy = -1; dy <= 1; ++dy) {
                                ...
                                if (!claimed_sources[candidate] &&
                                    visual_layer_matches(static_cast<viewport_visual_layer>(layer),
                                                         texpos, previous[candidate]) &&
                                    current[candidate] == 0) {
                                    source = candidate;
                                    ++candidate_count;
                                }
                    ...
                    for (size_t i = 0; i < existing_movement_count; ++i) {
                        ...
                        visual_source_x =
                            movement.source_x + (movement.target_x - movement.source_x) * progress;
                        visual_source_y =
                            movement.source_y + (movement.target_y - movement.source_y) * progress;
                        break;
                    }
                    state.movements.push_back({static_cast<viewport_visual_layer>(layer), texpos,
                                               visual_source_x, visual_source_y, x, y,
                                               frame_time_ms});
        ...
        // A source vacates its tile only if no movement this frame claimed it as a
        // target.
        ...
        std::remove_if(state.movements.begin(), state.movements.end(),
                       [&](const movementst &movement) {
                           ...
                           const int32_t target =
                               movement.target_x * input.dim_y + movement.target_y;
...
void visual_animation_managerst::end_frame() {
...
uint32_t visual_animation_managerst::get_frame_time_ms() const { return frame_time_ms; }

uint32_t visual_animation_managerst::get_frame_delta_ms() const { return frame_delta_ms; }

visual_facingst visual_animation_managerst::get_facing(const void *viewport, int32_t x,
                                                       int32_t y) const {
    for (const viewport_animationst &state : viewports) {
...
bool visual_animation_managerst::has_mirrored_facing(const void *viewport) const {
    for (const viewport_animationst &state : viewports) {
        if (state.viewport != viewport)
            continue;
...
bool visual_animation_managerst::requires_full_redraw() const { return force_full_redraw; }

bool visual_animation_managerst::has_active_movement(const void *viewport) const {
    for (const viewport_animationst &state : viewports) {
        if (state.viewport == viewport)
...
visual_movement_renderst visual_animation_managerst::get_movement(const void *viewport,
