...
void render(df::renderer_2d_base *renderer) { render_interpolated_world(renderer); }

bool flip_enabled() { return ::flip_enabled; }
...
bool zlevel_enabled() { return ::zlevel_enabled; }

void set_zlevel_enabled(bool enable) {
    ::zlevel_enabled = enable;
    if (gps != nullptr)
        ++gps->force_full_display_count;
}
