...
static void sendItemCreationEvents(color_ostream& out);
static void sendBuildingEvents(color_ostream& out);
static void sendBuildingCreatedEvents(color_ostream& out);
static void sendBuildingDestroyedEvents(color_ostream& out);
static void sendConstructionEvents(color_ostream& out);
static void sendConstructionAddedEvents(color_ostream& out);
static void sendConstructionRemovedEvents(color_ostream& out);
static void sendSyndromeEvents(color_ostream& out);
static void sendInvasionEvents(color_ostream& out);
static void sendInventoryChangeEvents(color_ostream& out);
static void sendReportEvents(color_ostream& out);
static void sendUnitAttackEvents(color_ostream& out);
static void manageUnloadEvent(color_ostream& out){};
static void sendInteractionEvents(color_ostream& out);
...
static std::map<int32_t,std::unordered_set<int32_t> > reportToRelevantUnits;
...
protected:
    void scan_units(color_ostream &out, const int32_t &tick) {
        ...
        // cleaning up stale id's
        for(auto iter = activeUnits.begin(); iter != activeUnits.end();) {
            auto id = iter->second;
            if (!all_units.count(id)) {
                iter = activeUnits.erase(iter);
            } else {
                ++iter;
            }
        }
        // cleaning up stale id's
        for(auto iter = deadUnits.begin(); iter != deadUnits.end();) {
            auto id = iter->second;
            if (!all_units.count(id)) {
                iter = deadUnits.erase(iter);
            } else {
                ++iter;
            }
        }
    ...
    void scan_items(color_ostream &out, const int32_t &tick){
        ...
        for (df::item* item : df::global::world->items.all) {
            ...
            if (!previous_items.count(id)) {
                ...
                newItems.emplace(tick, id);
            }
        }

        // clean up stale item id's
        for(auto iter = newItems.begin(); iter != newItems.end();) {
            // check whether a valid index for the id exists
            if (!valid_items.count(iter->second)) {
                newItems.erase(iter);
            } else {
                ++iter;
            }
    ...
    void scan_jobs(color_ostream &out, const int32_t &tick) {
        ...
        valid_jobs.clear(); // usually previous.swap() would be used to clear this... but that doesn't work here
        ...
        for (df::job_list_link* link = df::global::world->jobs.list.next; link != nullptr; link = link->next) {
            if (df::job* job = link->item) {
                auto id = job->id;
                // we need to retain this list for the next tick, so we can detect completed jobs
                // thus, we clone the jobs since they'll be gone if completed (usually)
...
static std::vector<df::unit*> gatherRelevantUnits(color_ostream& out, df::report* r1, df::report* r2) {
    ...
    auto pushbackReportUnits = [&](const df::report* report) {
        const auto &report_units = reportToRelevantUnits[report->id];
        if (report_units.size() > 2 && Once::doOnce("EventManager interaction too many relevant units")) {
            out.print("%s,%d: too many relevant units. On report\n \'%s\'\n", __FILE__, __LINE__, report->text.c_str());
        }
        for (int id: report_units) {
            if (ids.emplace(id).second) {
                result.push_back(df::unit::find(id));
            }
        }
    };
...
void DFHack::EventManager::manageEvents(color_ostream& out) {
    ...
    Scanner::get().scan(out, df::global::world->frame_counter);
    ...
    for (auto &key_value : copy) {
        ...
        if (tick - last_tick >= handler.freq) {
            ...
            for(;jter != newJobs.end();) {
                auto id = jter->second;
                if (Scanner::get().current_jobs.count(id)) {
                    // send the job* corresponding to the id
                    auto job = Scanner::get().current_jobs.find(id)->second;
...
static void sendBuildingEvents(color_ostream& out) {
    ...
    if (!df::global::building_next_id)
        return;
    sendBuildingCreatedEvents(out);
    sendBuildingDestroyedEvents(out);
}
